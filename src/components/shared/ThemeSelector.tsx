  ...
  return (
    <div className="flex items-center">
      <div className="block mr-1 items-center">
        <div title="The Rainbow Colors = Red, Orange, Yellow, Green, Blue, Indigo, Violet plus a few more. Just for fun! =)">
      ...
      <div className="relative inline-block w-20 text-center">
        <select ref={selectThemeRef} value={theme} onChange={handleThemeChange} className="block appearance-none w-full bg-content-bg border border-main-content-bg px-2 py-0.5 rounded shadow leading-tight focus:outline-none focus:shadow-outline text-text cursor-pointer">
          {Object.keys(Theme).filter(k => isNaN(Number(k))).map((k, i) => <option className="bg-content-bg border border-main-content" value={i} key={k}>{k}</option>)}
        </select>
