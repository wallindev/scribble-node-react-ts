...
const CustomButton: FC<ICustomButton> = ({ onClick, to, type = 'button', children, className, size = 'medium', ...props }): JSX.Element => {
  let classNamesButton = `inline-block outline-0 bg-button-bg text-button-text hover:bg-button-bg-hover focus-visible:bg-button-bg-hover border-0 rounded-sm cursor-pointer transition-colors duration-300`
  switch (size) {
    case 'small':
